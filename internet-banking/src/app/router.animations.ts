// Implementação futura, animar components ao carregar a página
...
export function fallIn() {
	return trigger('fallIn', [
		transition(':enter', [
			...
			animate('.5s .3s ease-in-out', style({ opacity: '1', transform: 'translateY(0)' }))
		]),
		transition(':leave', [
			style({ opacity: '1', transform: 'translateY(0)' }),
			animate('.5s ease-in-out', style({ opacity: '0', transform: 'translateY(40px)' }))
		])
	]);
}
